...
// Процедуры и функции, предназначение для заполнения этой формы с другой
&НаКлиенте
Процедура ЗаполнитьСКлиента(ДанныеЗаполнения) Экспорт
	ЗаполнитьССервера(ДанныеЗаполнения);	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьССервера(ДанныеЗаполнения)
	
	Документ = Документы.РеализацияАссортимента.СоздатьДокумент();
	Документ.Товар = ДанныеЗаполнения.Ключ;
	Документ.Количество = ДанныеЗаполнения.Значение;
