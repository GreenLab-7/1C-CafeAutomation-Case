...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	|ИЗ
	|	РегистрСведений.ЦеныПокупкиПродажи.СрезПоследних КАК ЦеныПокупкиПродажиСрезПоследних(&ДатаСреза)
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|	Документ.РеализацияАссортимента.Остатки КАК Остатки
	|		ПО
	|	Остатки.Ингредиент = ЦеныПокупкиПродажиСрезПоследних.Номенклатура
	|ГДЕ
	|	ЦеныПокупкиПродажиСрезПоследних.Номенклатура В (&Номеклатуры) И
	...
	ЗапросТоваров.УстановитьПараметр("Документ", ЭтотОбъект);
